    ...
    <div class="row justify-content-center">
        <div class="col-12">
            <div class="panel" style="background-color: rgba(0,0,0,0.5); color: #fff; padding: 20px; max-width: 90vw; margin: auto;">
                <div class="panel-header"><h2><?php echo e(__('messages.admin')); ?></h2></div>
                <div class="row">
                ...
                <div class="panel-body">
                    <?php if(count($registrations) > 0): ?>
                        <div class="row">
                            ...
                            <div id="id_card1_<?php echo e($reg->id); ?>" style="display: none;">
                                <span>
                                    ...
                                    <img src="http://localhost:8080/ereceptionhub/storage/app/public/mug_shots/<?php echo e($reg->avatar); ?>" style="position: absolute; width: 347px; left: 187px; top: 41px; border-radius: 50%;  border: solid 1px #eee;">
                                    <span style="position: absolute; color: #000; font-weight: 700; left: 590px; top: 87px; font-family: tahoma; font-size: 40px;"><p><?php echo e($reg->first_name); ?> <?php echo e($reg->last_name); ?></p></span>
                                    <img src="qr-code?text=<?php echo e($reg->rfid); ?>&size=300" alt="QR Code" style="position: absolute; left: 650px; top: 320px;">
                                    <span style="position: absolute; color: #000; font-weight: 700; left: 590px; top: 180px; max-width: 333px; font-family: tahoma; font-size: 40px;"><p><?php echo e($reg->job_title); ?></p></span>
                                    <?php 
                                    ...
                                    ?>
                                    <span style="position: absolute; color: #808080; font-weight: 700; left: 190px; top: 430px; max-width: 470px; font-family: tahoma; font-size: 40px;"><p><?php echo e($comp_name); ?></p></span>
                                </span>
                            </div>

                            <!-- ================  Access Pass Layout 380px WIDTH =====================  -->
                            <div id="id_card_<?php echo e($reg->id); ?>" style="display: none;">
                                <span>
                                    ...
                                    <?php endif; ?>
                                    <img src="http://localhost:8080/ereceptionhub/storage/app/public/mug_shots/<?php echo e($reg->avatar); ?>"
                                    ...
                                    <span style="margin: 30px; position: absolute; color: #000; font-weight: 700; left: 224px; top: 38px; font-family: tahoma; font-size: 15px;">
                                        <p><?php echo e($reg->first_name); ?> <?php echo e($reg->last_name); ?></p>
                                    </span>
                                    <img src="qr-code?text=<?php echo e($reg->rfid); ?>&size=115" alt="QR Code"
                                    ...
                                    <div class="col-6 col-sm-6 col-md-3 col-lg-3" style="background-color: rgba(0,0,0,0.1); padding: 10px; display: inline-block; border: solid 1px #aaa; margin-top: 3px;">
                                        <button onclick="printID(<?php echo e($reg->id); ?>)" class="btn btn-default btn-sm" style="position: absolute; top: 3px; right: 3px;"><i class="fa fa-print"></i> Print ID </button><br>
                                        <script>
                                            function printID(id){
                                        ...
                                        <img src="http://localhost:8080/ereceptionhub/storage/app/public/mug_shots/<?php echo e($reg->avatar); ?>" style="width: 60%; margin-left: 20%; border-radius: 50%;  border: solid 1px #eee;">
                                        <h6><span class="badge" style="background-color: #1e7553;">Name: </span> <?php echo e($reg->first_name); ?> <?php echo e($reg->last_name); ?> </h6>
                                        <h6><span class="badge" style="background-color: #1e7553;">Current Status: </span> <?php echo e($reg->current_status); ?> </h6>
                                        <hr>
                                        <p align="center"><a href="/ereceptionhub/public/administration/<?php echo e($reg->id); ?>" class="btn btn-primary brn-sm"><i class="fa fa-search"></i> View Full Details </a></p>
                                        <?php if(auth()->user()->user_level =='10'): ?>
                                            <a href="/ereceptionhub/public/administration/<?php echo e($reg->id); ?>/edit" class="btn btn-success btn-sm pull-left"><i class="fa fa-pencil"></i> Edit </a>
                                        <?php endif; ?>
                                        <span>
                                            <?php if(!Auth::guest()): ?>
                                            <?php if(Auth::user()->id != $reg->id): ?>                               
                                                <?php if(auth()->user()->user_level =='10'): ?>
                                                    <?php echo Form::open(['class' => "delete", 'action' => ['RegistrationsController@destroy', $reg->id], 'method' => 'POST', 'class' => 'pull-right']); ?>

                                                        <?php echo e(Form::hidden('_method', 'DELETE')); ?>

                                                        <button class="btn btn-danger btn-sm pull-right" onclick="return myConfirm();"><i class="fa fa-trash"></i> Delete </button>
                                                        <!--<?php echo e(Form::button('<i class="fa fa-trash"> </i> Delete ', ['class' => 'btn btn-danger btn-sm pull-right', 'type' => 'submit'])); ?>-->
                                                    <?php echo Form::close(); ?>
                                                ...
                                                var result = confirm("Are you sure you want to delete this registration?");
                                                    if (result==true) {
                                                        ...
                                                        return false;
                                                                                ...
                                                                                <!--<a href="/ereceptionhub/public/administration/<?php echo e($reg->id); ?>/delete" class="btn btn-danger btn-sm pull-right"><i class="fa fa-trash"></i> Delete </a>-->
                                        </span>
                                    </div>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </div>
...
<div class="modal fade" tabindex="-1" role="dialog" id="bulk_reg">
  ...
  <script>   
        function uploadCSV(){
            ...
            ajax.open("POST", "<?php echo e(route('bulk_reg.store')); ?>");
            ajax.send(formdata);
        }
...
<?php echo $__env->make('layouts.app2', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp2\htdocs\ereceptionhub\resources\views/admin/index.blade.php ENDPATH**/ ?>